...
namespace INMETRO.CIPP.SHARED.Servicos
{
    ...
    {
        public string[] ObterArquvosNoDiretorioRemotoSftp(FTPInfo ftp)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(ftp.Senha))
                {
                    tmpClient = new SftpClient(ftp.HostURI,Port, ftp.Usuario, ftp.Senha);
                }
                else
                {
                    tmpClient = new SftpClient(ftp.HostURI, Port, ftp.Usuario, "");
                }
                ...
                using (SftpClient client = tmpClient)
                {
                    ...
                    // was "/out"
                    IEnumerable<Renci.SshNet.Sftp.SftpFile> results = client.ListDirectory(ftp.DiretorioInspecao);
            ...
            {
                throw new Exception("Erro no servidor FTP");
            }
